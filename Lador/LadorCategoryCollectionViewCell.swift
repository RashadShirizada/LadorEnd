...
class LadorCategoryCollectionViewCell: UICollectionViewCell {
    ...
    func update(model:LadorCategory,cell:LadorCategoryCollectionViewCell){
        ...
        print("cell tag = \(cell.tag) and  frame height \(self.frame.height)")
        
        var image = UIImageView(frame:CGRect(x:self.frame.width/2 - 40 ,y:self.frame.height/2 - 60 ,width:80,height:80))
